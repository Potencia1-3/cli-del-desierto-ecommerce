...
import { getMyInfo, getMyProgress, createSession, getTimeSlots, getAvailableSlots, addReferral, getPackageTypes } from '../lib/api';
...
export default function ClientPortal() {
  ...
  const fetchData = async () => {
    try {
      const [infoRes, progressRes, slotsRes, typesRes] = await Promise.all([
        getMyInfo(),
        getMyProgress(),
        getTimeSlots(),
        getPackageTypes()
  ...
  const handleAddReferral = async () => {
    ...
    setSaving(true);
    try {
      await addReferral(clientInfo.id, referralForm);
      toast.success('¡Referido agregado! Gracias por recomendar Pump Fit');
      setReferralDialogOpen(false);
      setReferralForm({ name: '', phone: '', email: '', notes: '' });
      fetchData();
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Error al agregar referido');
    } finally {
      setSaving(false);
    }
  };
  ...
  const isProfileActive = clientInfo.profile_active;
  const referralsCount = clientInfo.referrals?.length || 0;
  const needsMoreReferrals = referralsCount < 3;
  ...
  return (
    <div className="min-h-screen bg-pf-background" data-testid="client-portal">
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-8 space-y-6">
        ...
        {!isProfileActive && (
          <Card className="bg-pf-warning/10 border-pf-warning/30 p-6">
            <div className="flex items-start gap-4">
              <Lock className="text-pf-warning flex-shrink-0" size={24} />
              <div>
                <h3 className="font-unbounded text-lg text-pf-warning mb-2">Perfil Pendiente de Activación</h3>
                <p className="text-pf-text-secondary mb-4">
                  Para poder agendar sesiones, necesitas completar tu registro:
                </p>
                <ol className="list-decimal list-inside text-pf-text-secondary space-y-2 mb-4">
                  <li>Pagar inscripción: <span className="text-white font-mono">{formatCurrency(packageTypes?.inscription_price || 599)}</span></li>
                  <li>Elegir un paquete de sesiones</li>
                  <li>Opcional: Plan de nutrición <span className="text-white font-mono">{formatCurrency(packageTypes?.nutrition_plan_price || 500)}</span></li>
                </ol>
                <p className="text-pf-secondary text-sm">
                  Contacta al administrador en recepción para completar tu pago y activar tu perfil.
                </p>
              </div>
            </div>
          </Card>
        ...
        {/* Referrals Reminder */}
        {needsMoreReferrals && isProfileActive && (
          <Card className="bg-pf-primary/10 border-pf-primary/30 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Gift className="text-pf-primary" size={20} />
                <div>
                  <p className="text-white font-medium">¡Recomienda a tus amigos!</p>
                  <p className="text-pf-text-secondary text-sm">
                    Agrega {3 - referralsCount} contacto(s) más para obtener beneficios exclusivos
                  </p>
                </div>
              </div>
              <Button onClick={() => setReferralDialogOpen(true)} className="btn-primary" size="sm">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-pf-text-secondary mt-1">
              {isProfileActive ? 'Bienvenido a tu portal personal' : 'Completa tu registro para comenzar'}
            </p>
          </div>
          
          {isProfileActive && clientInfo.active_packages?.length > 0 && (
            <Button onClick={() => setSessionDialogOpen(true)} className="btn-primary h-12" data-testid="portal-schedule-btn">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="glass-card p-4 text-center">
            ...
            <p className="text-pf-text-secondary text-xs">Referidos</p>
          </Card>
        </div>
        ...
        <Tabs defaultValue="sessions" className="space-y-6">
          ...
          <TabsContent value="sessions" className="space-y-6">
            {!isProfileActive ? (
              <Card className="glass-card p-8 text-center">
                <Lock className="mx-auto text-pf-warning mb-4" size={48} />
                <h3 className="text-white font-unbounded text-lg mb-2">Perfil No Activo</h3>
                <p className="text-pf-text-secondary">
                  Completa tu pago de inscripción y paquete para poder agendar sesiones.
                </p>
              </Card>
            ) : (
              <>
                  ...
                  {clientInfo.upcoming_sessions?.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      {clientInfo.active_packages?.length > 0 && (
                        <Button onClick={() => setSessionDialogOpen(true)} className="btn-primary mt-4">
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-pf-text-secondary text-sm">
                                {session.time} • Traje {session.suit_number} • 18 min
                              </p>
                            </div>
                          </div>
            ...
            <Card className="glass-card p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-white font-medium">Plan de Nutrición</h3>
                    <p className="text-pf-text-secondary text-sm">
                      {clientInfo.has_nutrition_plan 
                        ? 'Tienes acceso al plan de nutrición personalizado'
                        : `Mejora tus resultados por solo ${formatCurrency(packageTypes?.nutrition_plan_price || 500)}`
                      }
                    </p>
                  </div>
                </div>
                <Badge className={clientInfo.has_nutrition_plan ? 'badge-active' : 'badge-pending'}>
                  {clientInfo.has_nutrition_plan ? 'Activo' : 'No incluido'}
                </Badge>
              </div>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="packages" className="space-y-6">
            ...
            {packageTypes && (
              <Card className="glass-card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(packageTypes.packages || {}).map(([key, pkg]) => (
                    <div key={key} className="bg-pf-surface p-4 rounded-sm border border-pf-border relative overflow-hidden">
                      {pkg.promo_price < pkg.normal_price && (
                        <div className="absolute top-0 right-0 bg-pf-primary text-white text-xs px-3 py-1 rounded-bl-sm">
                          ¡PROMO!
                        </div>
                      )}
                      <h4 className="text-white font-medium mb-1">{pkg.name}</h4>
                      <p className="text-pf-text-secondary text-xs mb-2">{pkg.duration} • {pkg.max_reschedules} reagendamientos</p>
                      <div className="flex items-baseline gap-2">
                        <span className="text-pf-primary text-2xl font-unbounded">{formatCurrency(pkg.promo_price)}</span>
                        {pkg.promo_price < pkg.normal_price && (
                          <span className="text-pf-text-secondary text-sm line-through">{formatCurrency(pkg.normal_price)}</span>
                        )}
                      </div>
                    </div>
                  ))}
                </div>
                <p className="text-pf-text-secondary text-xs mt-4 text-center">
                  * Contacta a recepción para adquirir un paquete
                </p>
              </Card>
            )}
            ...
            {/* My Active Packages */}
            <Card className="glass-card p-6">
              <h2 className="font-unbounded text-lg text-white mb-4">Mis Paquetes Activos</h2>
              {clientInfo.active_packages?.length === 0 ? (
                <div className="text-center py-8">
                ...
                <div className="space-y-4">
                  {clientInfo.active_packages?.map((pkg) => (
                    <div key={pkg.id} className="bg-pf-surface p-4 rounded-sm border border-pf-border" data-testid={`portal-package-${pkg.id}`}>
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <h3 className="text-white font-unbounded">
                      ...
                      <div className="space-y-3">
                        ...
                        {/* Reschedule Info - Made clearer */}
                        <div className="bg-pf-background p-3 rounded-sm">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-pf-text-secondary text-xs">Reagendamientos disponibles</p>
                              <p className="text-white font-mono text-lg">
                                {pkg.max_reschedules - pkg.used_reschedules} <span className="text-pf-text-secondary text-sm">de {pkg.max_reschedules}</span>
                              </p>
                            </div>
                            {pkg.used_reschedules >= pkg.max_reschedules ? (
                              <Badge className="badge-error">Sin reagendamientos</Badge>
                            ) : (
                              <Badge className="badge-active">Disponibles</Badge>
                            )}
                          </div>
                          <p className="text-pf-text-secondary text-xs mt-2">
                            💡 Puedes reagendar una sesión si necesitas cambiar el horario
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <TabsContent value="referrals" className="space-y-6">
            <Card className="glass-card p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="font-unbounded text-lg text-white flex items-center gap-2">
                    <Gift className="text-pf-primary" size={20} />
                    Mis Referidos
                  </h2>
                  <p className="text-pf-text-secondary text-sm mt-1">
                    Recomienda a 3 amigos y obtén beneficios exclusivos
                  </p>
                </div>
                <Button onClick={() => setReferralDialogOpen(true)} className="btn-primary" data-testid="add-referral-btn">
                  <Plus size={18} className="mr-2" />
                  Agregar Referido
                </Button>
              </div>

              {/* Progress to 3 referrals */}
              <div className="bg-pf-surface p-4 rounded-sm mb-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-white font-mono">{referralsCount} / 3</span>
                </div>
                <div className="progress-bar">
                  <div 
                    className="progress-bar-fill" 
                ...
                {referralsCount >= 3 ? (
                  <p className="text-green-500 text-sm mt-2">🎉 ¡Felicidades! Has completado tus referidos</p>
                ) : (
                  <p className="text-pf-text-secondary text-sm mt-2">
                    Te faltan {3 - referralsCount} referido(s) para desbloquear beneficios
                  </p>
                )}
              </div>

              {/* Referrals List */}
              {clientInfo.referrals?.length === 0 ? (
                <div className="text-center py-8">
                  <Users className="mx-auto text-pf-text-secondary mb-4" size={48} />
                  <p className="text-pf-text-secondary mb-2">Aún no has agregado referidos</p>
                  <p className="text-pf-text-secondary text-sm">
                    Comparte Pump Fit con tus amigos y familiares
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {clientInfo.referrals?.map((ref, i) => (
                    <div key={ref.id || i} className="flex items-center justify-between p-4 bg-pf-background rounded-sm">
                      <div className="flex items-center gap-3">
                      ...
                      <Badge className={
                        ...
                        ref.status === 'contacted' ? 'badge-pending' :
                        'bg-white/10 text-white border border-white/20'
                      }>
                        {ref.status === 'converted' ? 'Inscrito' :
                         ref.status === 'contacted' ? 'Contactado' :
                         'Pendiente'}
                      </Badge>
                    </div>
                  ))}
          ...
          <TabsContent value="profile">
            <Card className="glass-card p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className={`p-4 rounded-sm border ${isProfileActive ? 'bg-green-500/10 border-green-500/30' : 'bg-pf-warning/10 border-pf-warning/30'}`}>
                  <p className="text-xs text-pf-text-secondary mb-1">Estado del Perfil</p>
                  <p className={`font-medium ${isProfileActive ? 'text-green-500' : 'text-pf-warning'}`}>
                    {isProfileActive ? '✓ Activo' : '⏳ Pendiente de activación'}
                  </p>
                </div>
                <div className={`p-4 rounded-sm border ${clientInfo.inscription_paid ? 'bg-green-500/10 border-green-500/30' : 'bg-pf-error/10 border-pf-error/30'}`}>
                  <p className="text-xs text-pf-text-secondary mb-1">Inscripción</p>
                  <p className={`font-medium ${clientInfo.inscription_paid ? 'text-green-500' : 'text-pf-error'}`}>
                    {clientInfo.inscription_paid ? '✓ Pagada' : '✗ Pendiente'}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-pf-primary/10 rounded-sm border border-pf-primary/20">
                <p className="text-pf-primary text-sm">
                  Para actualizar tu información o completar tu registro, contacta a recepción.
                </p>
              </div>
            </Card>
          </TabsContent>
      ...
      <Dialog open={sessionDialogOpen} onOpenChange={setSessionDialogOpen}>
        <DialogContent className="glass-card border-pf-border">
          ...
          <div className="space-y-4 mt-4">
            ...
            {sessionForm.date && (
              <div className="space-y-2">
                <Label className="text-pf-text-secondary">Horario Disponible (intervalos de 30 min)</Label>
                    ...
                    <SelectContent className="bg-pf-surface border-pf-border max-h-60">
                      {availableTimes.map((slot) => (
                        <SelectItem key={slot} value={slot} className="text-white hover:bg-pf-primary/20 font-mono">
                          {slot} ({availableSlots[slot]?.length} traje{availableSlots[slot]?.length > 1 ? 's' : ''} disponible{availableSlots[slot]?.length > 1 ? 's' : ''})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
            ...
            <div className="bg-pf-surface p-3 rounded-sm text-sm">
              <p className="text-pf-text-secondary">
                ⏱️ Duración de la sesión: <span className="text-white">18 minutos</span>
              </p>
              <p className="text-pf-text-secondary mt-1">
                📅 Máximo: <span className="text-white">2 sesiones por semana</span>
              </p>
            </div>
      ...
      {/* Agregar Referido Dialog */}
      <Dialog open={referralDialogOpen} onOpenChange={setReferralDialogOpen}>
        <DialogContent className="glass-card border-pf-border">
          <DialogHeader>
            <DialogTitle className="font-unbounded text-white">Agregar Referido</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <p className="text-pf-text-secondary text-sm">
              Recomienda a tus amigos y familiares para obtener beneficios exclusivos.
            </p>
            
            <div className="space-y-2">
              <Label className="text-pf-text-secondary">Nombre del contacto *</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="text-pf-text-secondary">Teléfono / WhatsApp *</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="text-pf-text-secondary">Email (opcional)</Label>
              <Input
                type="email"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ej: Interesado en bajar de peso"
                className="input-dark"
                data-testid="referral-notes-input"
              />
            </div>

            <div className="flex gap-3 justify-end pt-4">
              <Button variant="ghost" onClick={() => setReferralDialogOpen(false)} className="btn-ghost">
                Cancelar
              </Button>
              <Button 
                onClick={handleAddReferral} 
                className="btn-primary" 
                disabled={saving || !referralForm.name || !referralForm.phone}
                data-testid="save-referral-btn"
              >
                {saving ? 'Guardando...' : 'Agregar Referido'}
              </Button>
            </div>
          </div>
        </DialogContent>
